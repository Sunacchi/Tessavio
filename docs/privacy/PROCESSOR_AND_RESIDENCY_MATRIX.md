...
| OAuth OpenRouter          | code PKCE e API key OpenRouter controllata dall'utente        | OpenRouter                                                   | nessuna garanzia EU implicita; eventuale prodotto/endpoint EU va contrattualmente verificato | Phase C: cifratura, revoca, DPA/subprocessori, region routing, ZDR e budget                  |
| OpenRouter -> provider AI | contesto minimo necessario e output strutturato               | provider effettivamente selezionato e relativi subprocessori | dipende dall'endpoint/provider, non dal D1                                                   | allowlist per regione/privacy, ZDR/no-training, retention e fallback verificati per endpoint |
